...
class LeaderBoard implements IDBAccessObject {
    ...
    public function displayLeaderBoard($userId,$userPoints){
        global $wgPhase2Images, $wgProfilePage, $wgLeaderBoardPointScale, $wgBadges;
        $html="";
        if (is_int($userId) && $userId>=0 && is_int($userPoints) && $userPoints>0){
            ...
            while ($myrow=$mydbr->fetchRow($myres)){
                    ...
                    if ($addedPost==0){
                       ...
                       $html.="<td><a title=\"Click link to view\" href=\"$wgProfilePage?user=".$currentUser."&add=1\">".$addedPost."</a></td>"; 
                    }
                    if ($editedPost==0){
                        $html.="<td>$editedPost</td>";
                    }
                    else{
                       $html.="<td><a title=\"Click link to view\" href=\"$wgProfilePage?user=".$currentUser."&edit=1\">".$editedPost."</a></td>"; 
                    }     
                    $html.="<td>".$points."</td>";
                    $html.="<td>";
                    ...
                    foreach($badgesEarned as $badge){
                        $badgeValue=$wgBadges[$badge];
                        ++$counter;
                        if ($badge==$previousBadge || $counter==1){
                             $html.="<img src=\"".$wgPhase2Images.$badge.".png\" alt=\"".$badge."\" title=\"".ucfirst($badge)." (".$badgeValue." pts)\" width=\"18\" height=\"25\">";
                        }
                        else{                                            
                             $html.="<br/><img src=\"".$wgPhase2Images.$badge.".png\" alt=\"".$badge."\" title=\"".ucfirst($badge)." (".$badgeValue." pts)\" width=\"18\" height=\"25\">";                       
                        }
                        $previousBadge=$badge;
                    }
                    $html.="</td></tr>";
